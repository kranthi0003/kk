...
const statusStyles = {
  active: { label: 'Active', dot: 'bg-green-500', text: 'text-green-500', ring: 'border-green-500/25 bg-green-500/10' },
  wip: { label: 'WIP', dot: 'bg-orange-500', text: 'text-orange-500', ring: 'border-orange-500/25 bg-orange-500/10' },
  research: { label: 'Proposed', dot: 'bg-accent', text: 'text-accent', ring: 'border-accent/25 bg-accent/10' },
  archived: { label: 'Archived', dot: 'bg-muted-foreground', text: 'text-muted-foreground', ring: 'border-border bg-muted/40' },
}

const tints = ['pr-tint-violet', 'pr-tint-magenta', 'pr-tint-coral']

function Card({ p, i, reserveTitle = false }) {
  const s = statusStyles[p.status] ?? statusStyles.archived
  const hasLinks = Boolean(p.github || p.url)
  return (
    <div
      className={`reveal hover-lift group relative flex h-full flex-col bg-card hover:border-accent/40 transition-colors overflow-hidden ${tints[i % 3]}`}
    >
      ...
      {/* Meta row: slug identifies the card, the pill says how real it is. */}
      <div className="flex items-center gap-3 px-5 pt-4 pb-3">
        <span className="min-w-0 flex-1 truncate font-mono text-[11px] tracking-wide text-muted-foreground">{p.name}</span>
        <span className={`inline-flex flex-shrink-0 items-center gap-1.5 rounded-full border px-2 py-[3px] text-[9px] font-semibold uppercase tracking-[0.14em] ${s.ring} ${s.text}`}>
      ...
      <div className="flex flex-1 flex-col px-5 pb-5">
        {/* Research titles vary between one and two lines, so reserve two to keep
            the questions below them on a shared baseline. Shipped titles all fit
            one line, so reserving there would only add dead space. */}
        <h3 className={`font-heading text-[15px] font-semibold leading-snug text-foreground ${reserveTitle ? 'min-h-[2.6rem]' : ''}`}>
          {p.title}
        </h3>

        {/* On a research card the question is the point, so it leads. */}
        {p.question && (
          <p className="mt-1 border-l-2 border-accent/40 pl-3 text-xs italic leading-relaxed text-foreground/85">
            {p.question}
          </p>
        )}

        <p className={`mt-3 text-xs leading-relaxed text-muted-foreground ${p.question ? '' : 'line-clamp-3'}`}>{p.desc}</p>

        {/* mt-auto pins the footer so chips align across every card in a row. */}
          ...
          <div className="flex items-end justify-between gap-3 border-t border-border/40 pt-3">
            ...
            {hasLinks && (
              <div className="flex items-center gap-2.5 flex-shrink-0 pb-0.5">
                {p.github && (
                  <a href={p.github} target="_blank" rel="noopener noreferrer" aria-label={`${p.title} on GitHub`} className="text-muted-foreground hover:text-accent transition-colors">
                ...
                {p.url && (
                  <a href={p.url} target="_blank" rel="noopener noreferrer" aria-label={`Open ${p.title}`} className="text-muted-foreground hover:text-accent transition-colors">
  ...
  return (
    <section id="projects" className="py-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {research.map((p, i) => <Card key={p.name} p={p} i={i} reserveTitle />)}
        </div>
