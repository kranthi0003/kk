        ...
        <div className="space-y-4">
          {projects.map((p) => {
            ...
            return (
              <div
                key={p.name}
                className="group rounded-xl border border-border/30 bg-card hover:border-border/60 transition-all duration-200"
              >
                {/* Title bar — OS window style */}
                <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/20 bg-muted/30 rounded-t-xl">
                  <div className="flex gap-1.5">
                    <span className="w-2.5 h-2.5 rounded-full bg-[#ff5f57]" />
                    <span className="w-2.5 h-2.5 rounded-full bg-[#febc2e]" />
                    <span className="w-2.5 h-2.5 rounded-full bg-[#28c840]" />
                  </div>
                  <span className="flex-1 text-center text-[11px] font-mono text-muted-foreground">{p.name}</span>
                ...
                <div className="px-5 py-4">
                  ...
                  <div className="font-mono text-sm space-y-1.5">
                    ...
                    <p className="text-muted-foreground text-[13px] leading-relaxed pl-5">{p.desc}</p>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-border/10">
                    <div className="flex flex-wrap gap-1.5">
                      {p.tech.map(t => (
                        <span key={t} className="px-2 py-0.5 rounded text-[10px] font-mono bg-muted/60 text-muted-foreground border border-border/20">
                          {t}
                        </span>
                      ))}
                    </div>
                    <div className="flex items-center gap-3">
                      {p.github && (
                        <a href={p.github} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" title="Source">
                      ...
                      {p.url && (
                        <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" title="Live">
