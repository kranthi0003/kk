...
// ─── Interactive Cosmic Dust — particles that react to mouse ─
function CosmicDust({ count = 3000 }) {
  const ref = useRef()
  ...
  const { positions, velocities, basePositions, sizes, colors } = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      // Distribute in a large sphere around the solar system
      ...
      // Subtle color variation: white → light blue → pale violet
      const hue = Math.random()
      if (hue < 0.4) { col[i*3]=0.7; col[i*3+1]=0.75; col[i*3+2]=1.0 }       // blue-white
      else if (hue < 0.7) { col[i*3]=0.85; col[i*3+1]=0.7; col[i*3+2]=1.0 }   // pale violet
      else { col[i*3]=1.0; col[i*3+1]=0.95; col[i*3+2]=0.85 }                  // warm white
    }
    return { positions: pos, velocities: vel, basePositions: base, sizes: sz, colors: col }
  }, [count])
  ...
  useFrame((state, delta) => {
    ...
    // Project mouse to a plane at z=0 in world space for interaction
    raycaster.setFromCamera(pointer, camera)
    const plane = new THREE.Plane(new THREE.Vector3(0, 1, 0), 0)
    const intersection = new THREE.Vector3()
    raycaster.ray.intersectPlane(plane, intersection)
    if (intersection) {
      prevMouse3D.current.copy(mouse3D.current)
      mouse3D.current.copy(intersection)
      mouseVelocity.current.subVectors(mouse3D.current, prevMouse3D.current).multiplyScalar(1 / Math.max(dt, 0.001))
    }

    const mx = mouse3D.current.x
    const my = mouse3D.current.y
    const mz = mouse3D.current.z
    const mvx = mouseVelocity.current.x
    const mvy = mouseVelocity.current.y
    const mvz = mouseVelocity.current.z
    const mouseSpeed = Math.sqrt(mvx * mvx + mvy * mvy + mvz * mvz)

    const INFLUENCE_RADIUS = 25
    const INFLUENCE_SQ = INFLUENCE_RADIUS * INFLUENCE_RADIUS
    const PUSH_STRENGTH = 180
    const DRAG_STRENGTH = mouseSpeed > 5 ? 0.4 : 0.0 // drag particles along when fast
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (distSq < INFLUENCE_SQ && distSq > 0.01) {
        ...
        const pushForce = PUSH_STRENGTH * falloffSq * dt
        const nx = dx / dist
        const ny = dy / dist
        const nz = dz / dist
        ...
        // Drag along with mouse velocity (fluid-like)
        if (DRAG_STRENGTH > 0) {
          velocities[i3] += mvx * DRAG_STRENGTH * falloffSq * dt
          velocities[i3 + 1] += mvy * DRAG_STRENGTH * falloffSq * dt
          velocities[i3 + 2] += mvz * DRAG_STRENGTH * falloffSq * dt
        }
      }

      // Spring back to base position
      velocities[i3] += (bx - px) * RETURN_SPEED * dt
      velocities[i3 + 1] += (by - py) * RETURN_SPEED * dt
      velocities[i3 + 2] += (bz - pz) * RETURN_SPEED * dt
      ...
      // Integrate
      arr[i3] = px + velocities[i3] * dt
      arr[i3 + 1] = py + velocities[i3 + 1] * dt
      arr[i3 + 2] = pz + velocities[i3 + 2] * dt
    }
