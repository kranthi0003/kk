  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-24 md:pt-28 overflow-hidden">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 py-6 md:py-8 lg:py-12 text-center w-full">
        {/* Profile — quieter ring, no pulsing aura */}
        ...
        {/* Eyebrow — provisionr signature small caps */}
        ...
        {/* Headline — large, quiet weight, tight tracking */}
        <h1
        ...
        {/* CTAs — provisionr rounded-md style */}
        ...
        <div className="mt-12 grid grid-cols-3 gap-px max-w-2xl mx-auto rounded-lg border border-border/60 overflow-hidden bg-border/40 animate-fade-in-up" style={{ animationDelay: '0.6s' }}>
          {[
            { value: '5+', label: 'Years building infra' },
            { value: '100+', label: 'Services & tools shipped' },
            { value: '24/7', label: 'On-call rotation veteran' },
          ].map(s => (
            <div key={s.label} className="bg-card/40 backdrop-blur px-4 py-4">
              <div className="text-2xl md:text-3xl font-heading font-semibold tracking-tight text-foreground">{s.value}</div>
